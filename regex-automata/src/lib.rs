...
An example of configurable failure happens when enables heuristic support for
Unicode word boundaries in a DFA. Namely, since the DFAs in this crate (except
for the one-pass DFA) do not support Unicode word boundaries on non-ASCII
haystacks, building a DFA from an NFA that contains a Unicode word boundary
will itself fail. However, one can configure DFAs to still be built in this
case by
...
There are multiple size limit options in this crate. If one is relevant for the
object you're building, it will be on method on a corresponding `Config` type.
