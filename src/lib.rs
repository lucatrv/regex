...
The principle type in this crate is a [`Regex`]. Its principle methods are
as follows:
...
one shouldn't expect to observe this unless the principle task in each thread
...
The principle way this crate deals with them is by limiting their size by
...
the regex size limit above will protect you from the most egregious cases, the
